...
class CopyBranchLinkAction : AnAction(), DumbAware {
    ...
    override fun actionPerformed(e: AnActionEvent) {
        ...
        if (branchUrl != null) {
            CopyPasteManager.getInstance().setContents(StringSelection(branchUrl))
            showSuccessNotification(project, branchUrl)
        } else {
            showNoRemoteNotification(project)
        }
    }
    ...
    private fun showSuccessNotification(project: Project, url: String) {
        val notification =
            Notification(
                "IssueLinker",
                IssueLinkerBundle.message("action.copyBranchLink.success", url),
                "",
